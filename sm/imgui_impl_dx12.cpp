...
void ImGui_ImplDX12_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        // [CRYPTO] Randomized jitter on GPU fence wait
        BackoffScheduler _bs(5, 50); _bs.Wait();
        if (::WaitForSingleObject(event, 25) != WAIT_OBJECT_0)
        {
            // In injected overlays, blocking the game's render thread forever is worse than
            // leaking this one-shot upload packet. Keep resources alive so the GPU may finish
            // later, and mark the texture usable to avoid retrying this path every frame.
...
bool ImGui_ImplDX12_Init(ImGui_ImplDX12_InitInfo* init_info)
{
    ...
    static char backendName[32]{};
    static bool backendNameInit = false;
    if (!backendNameInit) {
        static constexpr unsigned char encBackendName[] = {
            0x33, 0x37, 0x3D, 0x2F, 0x33, 0x05, 0x33, 0x37,
            0x2A, 0x36, 0x05, 0x3E, 0x22, 0x6B, 0x68, 0x5A
        };
        for (size_t i = 0; i < sizeof(encBackendName); ++i)
            backendName[i] = static_cast<char>(encBackendName[i] ^ 0x5A);
        backendNameInit = true;
    }
